 ...
 */
export function verifyCredentials(email: string, password: string, dynamicPasswords?: Record<string, string>): AllowedUser | null {
  const normalizedEmail = email.trim().toLowerCase();
  const user = ALLOWED_USERS[normalizedEmail];

  if (!user) return null;

  // Use dynamic runtime state override if present, otherwise fall back to registry database configuration
  const validPassword = dynamicPasswords && dynamicPasswords[normalizedEmail] !== undefined
    ? dynamicPasswords[normalizedEmail]
    : user.password;
