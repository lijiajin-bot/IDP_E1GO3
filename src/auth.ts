...
export default function LoginPage({ onLogin }: LoginPageProps) {
  const { resetUserPassword, dynamicPasswords } = useAppState();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // 1. First check if a dynamic password reset exists in context state memory
    if (dynamicPasswords && dynamicPasswords[targetEmail]) {
      if (dynamicPasswords[targetEmail] === password.trim()) {
        // Fallback or derive role profile structure matching standard AllowedUser patterns
        const isStaff = targetEmail.endsWith('staff@utm.my') || !targetEmail.includes('graduate');
        onLogin({
          email: targetEmail,
    ...
    // 2. Fallback to original hardcoded mock registry if no updated entries match
